...
class NFCE():
    ...
    def abre_nfce_com_xml(self):
        diretorio = '/u1/caixa/dev/tty1/header.xml'
    ...
    def item_nfce_com_xml(self):
        diretorio = '/u1/caixa/dev/tty1/itens.xml'
        xml_string = self.ler_arquivo_xml(diretorio)
	...
	if status not in (1, 2, 3):
	    ...
	    # 3- Emitida em contingencia onLine;
            # eddie -- fazer o teste para capturar a mensagem de erro aqui
            strCodigo = " " * 4
            strMensagem = " " * 600
            status02 = self.dll.rAvisoErro_NFCe_Daruma(strCodigo, strMensagem)
            if status02 == 1:
                raise Exception(strMensagem)
